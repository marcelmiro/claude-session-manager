...
**`claude0 setup` details**: Installs Claude0's Claude lifecycle hooks under `~/.config/claude0/hooks`, preserving existing hooks and settings. It also updates narrowly scoped Claude0 tmux/zsh fragments under `~/.config/claude0/`, installs the `~/.local/bin/claude0` and `~/.local/bin/c0` commands plus a private terminal launcher under `~/.config/claude0/`, and adds one import to the user's `.tmux.conf` and `.zshrc`. Safe to run multiple times (idempotent); personal dotfiles, prompts, tmux presentation, and TPM-managed plugins are never replaced or installed.
...
- **View toggle** (transitional): `localStorage["claude0-view"]`, default `"inbox"`; the classic repo-grouped list renders unchanged behind it. The navbar ⚡/🔄 chips are gone in inbox view — section header counts carry that answer (the 🤖 pill stays).

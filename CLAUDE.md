...
- **Model** (`core/inbox-model.ts`, pure): wake math (hours exact, days at local midnight, local calendar not UTC), section derivation (`sectionOf`/`effectiveSince`/`deriveSections`), snapshot+overlay composition. View building is pure too (`src/sidebar/rows.ts`) and input decoding (`src/sidebar/input.ts`) — both tested without tmux.

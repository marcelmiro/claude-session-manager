...
Sessions carry a lifecycle (Needs you → Running → Parked → Recently done); a per-window tmux sidebar is the surface, `csm daemon` is the engine. Decision record: [ADR 13](docs/adr/0013-inbox-lifecycle-model.md) + its addenda (the second is the settled interaction grammar; the sixth is the portkey inbox).
...
- Interaction grammar (keys, snooze digits-then-unit, disposition walk, select-then-commit clicks, section-scoped verbs) is documented in ADR 0013 addendum 2 and `prototype/inbox-sidebar/README.md`.

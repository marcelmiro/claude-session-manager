...
`!cmd` writes two adjacent user JSONL records (`<bash-input>`, then `<bash-stdout>/<bash-stderr>`); the parser folds them into one `bash` turn (`foldBashTurns` in `core/transcript.ts`), rendered terminal-style — right-aligned peach mono command bubble, output as a mint/red rail with an 8-line clamp. Optimistic/queued `!` sends render the same bubble and retire against `"!" + command`. Bash turns ARE rewind checkpoints (Claude's picker lists them — the deliberate opposite of slash-command turns). The composer flips into **bash mode** on a typed `!` opening an empty draft (glyph in-field, textarea remounted with autocorrect off — iOS honors the attributes only at focus time); a paste never flips it (terminal parity — the pane treats a pasted `!cmd` as literal text too). `sendMessage` pre-flights `shellModeInput` (`core/session-api.ts`): a queued bang leaves the pane's prompt IN shell mode, where the `❯`-keyed draft guard is blind and a plain send would execute as bash — an empty shell prompt is cleared with `BSpace` + re-verify, one holding text aborts with reason `shell-draft` (no kill-ring gamble). Decision record: [ADR 12](docs/adr/0012-bang-commands.md).

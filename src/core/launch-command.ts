 ...
 * setup (worktree add / checkout, if any) then `claude`. Pure — returns the
 * string, runs nothing.
 *
...
export function buildLaunchCommand(
  ...
  if (mode === "new-branch") {
    ...
    return `${fetchPrefix}{ git worktree add ${shellQuote(wtAbs)} -b ${shellQuote(text)} --end-of-options ${shellQuote(baseRef)} 2>/dev/null || git worktree add ${shellQuote(wtAbs)} ${shellQuote(text)}; } && cd ${shellQuote(wtAbs)} && claude`;
  }
  ...
  if (mode === "reuse") {
    ...
    return `${fetchPrefix}git worktree add ${shellQuote(wtAbs)} ${shellQuote(branch.name)} && cd ${shellQuote(wtAbs)} && claude`;
  }
  ...
  if (mode === "checkout") {
    ...
    return `${fetchPrefix}${checkout} && claude`;
  }

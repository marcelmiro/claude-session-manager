...
`csm-terminal` owns the client connection and uses Mosh for roaming and sleep/wake resilience. The selected host and local/remote mode are machine-local state under `~/.config/csm/`, so dotfiles contain no personal tailnet hostname. Plain SSH remains the diagnostic fallback; tmux owns session persistence either way. Eternal Terminal was prototyped during the migration and retired before cutover.
